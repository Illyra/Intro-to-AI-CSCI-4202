...
problem = JSON.parsefile("1-move.json")
...
function Backtrack1(problem, datalist, bound)
    ...
    count += 1
    data = datalist.head
    data in datalist.tail && return nothing
    data == problem["goal"] && return nil(Rule)
    length(datalist) > bound && return nothing
    for rule in applicable_rules(data)
        result = Backtrack1(problem, cons(rule.action(data), datalist), bound)
        isnothing(result) || return cons(rule, result)
    end
    return nothing
end
...
function run_once(problem, depth)
    ...
    solution = Backtrack1(problem, cons(problem["start"], nil()), depth)
...
function run_many(problem, limit)
    ...
    for i = 0:limit
        solution = Backtrack1(problem, cons(problem["start"], nil()), i)
